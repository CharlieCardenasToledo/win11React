...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildSlidePreview(slide, index) {
        const heading =
        ...
        const sub =
            ...
            slide.querySelector('li')?.textContent?.trim() ||
            '';
        const bullets = Array.from(slide.querySelectorAll('li'))
            .slice(0, 3)
    ...
    function notifyParent(type = 'state') {
        if (thumbMode) return;
        ...
        const payload = {
            ...
            total: totalSlides,
            titles: slides.map((slide, index) => slide.getAttribute('data-title') || `Slide ${index + 1}`),
            previews: slides.map((slide, index) => buildSlidePreview(slide, index)),
        };
    ...
    function showSlide(index) {
        ...
        notifyParent('slide-change');
    }

    // Botones (solo modo standalone)
    if (prevBtn) {
    ...
    // Teclado: flechas, espacio, PgUp/PgDown (solo standalone)
    if (!embedded && !thumbMode) {
    ...
    // Comandos desde el contenedor (PowerPoint React)
    window.addEventListener('message', (event) => {
        const data = event.data || {};
        if (data.type !== 'ppt-command') return;
        if (thumbMode) return;
        if (data.command === 'next') showSlide(currentIndex + 1);
        if (data.command === 'prev') showSlide(currentIndex - 1);
        if (data.command === 'go' && Number.isInteger(data.index)) showSlide(data.index);
        if (data.command === 'state') notifyParent('state-request');
    });

    if (embedded) {
        if (indexModal) indexModal.remove();
        if (indexBtn) indexBtn.remove();
        if (prevBtn) prevBtn.remove();
        if (nextBtn) nextBtn.remove();
        if (currentSlideSpan) currentSlideSpan.parentElement?.remove();
        if (progressBar) progressBar.parentElement?.remove();
    }

    if (thumbMode) {
        // For thumbnail rendering: force one slide, no UI controls.
        if (indexModal) indexModal.remove();
        if (indexBtn) indexBtn.remove();
        if (prevBtn) prevBtn.remove();
        if (nextBtn) nextBtn.remove();
        if (currentSlideSpan) currentSlideSpan.parentElement?.remove();
        if (progressBar) progressBar.parentElement?.remove();
        if (document.getElementById('theme-toggle')) document.getElementById('theme-toggle').remove();
        const safeIndex = Number.isInteger(requestedSlide) ? Math.min(totalSlides - 1, Math.max(0, requestedSlide - 1)) : 0;
        showSlide(safeIndex);
        return;
    }
